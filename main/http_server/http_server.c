...
esp_err_t POST_WWW_update(httpd_req_t * req)
{
    ...
    if (!bzm_ota_guard_retain_for_reboot(&ota_guard)) {
        ...
        return bzm_ota_error_after_release(
            req, &ota_guard, HTTPD_500_INTERNAL_SERVER_ERROR,
            "Unable to retain Bonanza WWW update ownership for reboot");
    }
...
esp_err_t start_rest_server(GlobalState * global_state)
{
    ...
    /* Keep the recovery API available even when the bridge firmware is not
     * responsive. Board identity, not a successful UART probe, controls route
     * registration. */
