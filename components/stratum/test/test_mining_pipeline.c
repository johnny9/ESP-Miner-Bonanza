...
TEST_CASE("Common work retries retain extranonce and the clean generation", "[mining][common-work]")
{
    miner_job_t *job = prepare_followup_job(1, false);
    job->work_generation = 7;
    job->clean_jobs = false; // A coalesced notify must retain the new generation boundary.
    const job_pipeline_harness_event_t events[] = {
        {.type = JOB_PIPELINE_HARNESS_NOTIFY, .slot = 0},
        {.type = JOB_PIPELINE_HARNESS_TIMEOUT},
    };
    job_pipeline_harness_result_t result;
    job_pipeline_harness_run((job_pipeline_harness_config_t) {
        .hardware_version_rolling = true, .asic_initialized = true,
...
TEST_CASE("Common work rejects a retired pool session before allocation", "[mining][common-work]")
{
    miner_job_t *job = prepare_followup_job(1, false);
    job->work_generation = 7;
    ...
    job_pipeline_harness_run((job_pipeline_harness_config_t) {
        .hardware_version_rolling = true, .asic_initialized = true,
        .job_frequency_ms = 1, .current_generation = 8,
    }, &event, 1, &result);
