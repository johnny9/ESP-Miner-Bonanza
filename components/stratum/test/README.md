...
| `job_test_state.h`, `stubs/global_state.h` | Selects the real QEMU state or an explicit native-only state view, without exposing it to unrelated tests. |
...
| `test_job_building.c` | Coinbase hashing at stack and heap limits, explicit failure status, zeroed output, and allocation recovery. |
...
| `test_stratum_json.c` | SV1 JSON-RPC parsing, job fields, server messages, malformed input, and size limits. ESP transport framing lives in `test_sv1_transport.c`. |
