          ...
          "[mining][asic-job]")
{
    miner_job_t source = {
        ...
        .pool_diff = -1.0, .pool_id = 255, .extranonce2_len = 0,
    };
...
TEST_CASE("coinbase allocation failure preserves the destination",
          "[mining][asic-job]")
{
    ...
    const asic_job_t original = job;
    mining_allocator_fault_injector_reset(1);
    TEST_ASSERT_FALSE(mining_build_asic_job(&source, 0, 0, &job));
