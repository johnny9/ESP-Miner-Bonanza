...
TEST_CASE("Encode SV1 setup requests", "[sv1 protocol]")
{
    ...
    int length = STRATUM_V1_encode_configure_version_rolling(message, sizeof(message), 1, 0x1fffe000);
    assert_encoded_message(
        "{\"id\":1,\"method\":\"mining.configure\",\"params\":[[\"version-rolling\"],{\"version-rolling.mask\":\"1fffe000\",\"version-rolling.min-bit-count\":0}]}\n",
        length, message);
...
TEST_CASE("Encode SV1 share submission", "[sv1 protocol]")
{
    ...
    int length = STRATUM_V1_encode_submit_share(
        ...
        0x01234567, 0x89abcdef, &(uint32_t){0x00002000});
...
TEST_CASE("Reject invalid SV1 encoder output buffers", "[sv1 protocol]")
{
    ...
    TEST_ASSERT_EQUAL_INT(-1, STRATUM_V1_encode_configure_version_rolling(NULL, 1, 1, 0x1fffe000));
    TEST_ASSERT_EQUAL_INT(-1, STRATUM_V1_encode_configure_version_rolling(&message, 0, 1, 0x1fffe000));
}
...
TEST_CASE("Reject every truncated SV1 message", "[sv1 protocol]")
{
    ...
    assert_encode_did_not_fit(
        ...
        message);
    assert_encode_did_not_fit(
        STRATUM_V1_encode_configure_version_rolling(message, sizeof(message), 1, 0x1fffe000), message);
}
...
TEST_CASE("Escape every SV1 JSON string field", "[sv1 protocol]")
{
    ...
    length = STRATUM_V1_encode_submit_share(
        ...
        0, UINT32_MAX, &(uint32_t){0x00002000});
    assert_encoded_message(
...
TEST_CASE("Encode SV1 messages at exact buffer boundaries", "[sv1 protocol]")
{
    ...
    ASSERT_EXACT_CAPACITY(STRATUM_V1_encode_submit_share(
        message, capacity, INT_MAX, "worker", "job", "", 0, UINT32_MAX, &(uint32_t){UINT32_MAX}));
    ASSERT_EXACT_CAPACITY(STRATUM_V1_encode_configure_version_rolling(
        message, capacity, INT_MIN, 0x1fffe000));
}

TEST_CASE("Encode optional SV1 version bits including an accepted zero mask", "[sv1 protocol]")
{
    char message[256];
    int length = STRATUM_V1_encode_submit_share(
        message, sizeof(message), 4, "worker", "job", "", 0, 1, NULL);
    assert_encoded_message(
        "{\"id\":4,\"method\":\"mining.submit\",\"params\":[\"worker\",\"job\",\"\",\"00000000\",\"00000001\"]}\n", length, message);
    length = STRATUM_V1_encode_submit_share(
        message, sizeof(message), 4, "worker", "job", "", 0, 1, &(uint32_t){0});
    assert_encoded_message(
        "{\"id\":4,\"method\":\"mining.submit\",\"params\":[\"worker\",\"job\",\"\",\"00000000\",\"00000001\",\"00000000\"]}\n", length, message);
}
