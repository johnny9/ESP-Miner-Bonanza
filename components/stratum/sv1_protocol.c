...
int STRATUM_V1_encode_submit_share(char *buffer, size_t capacity, int message_id,
                                   ...
                                   uint32_t nonce, const uint32_t *version_bits)
{
    json_writer_t writer;
    if (!json_writer_init(&writer, buffer, capacity)) return -1;
    ...
    json_writer_append_format(&writer, ",\"%08" PRIx32 "\",\"%08" PRIx32 "\"", ntime, nonce);
    if (version_bits != NULL) {
        json_writer_append_format(&writer, ",\"%08" PRIx32 "\"", *version_bits);
    }
    json_writer_append_literal(&writer, "]}\n");
    return json_writer_finish(&writer);
}

int STRATUM_V1_encode_configure_version_rolling(char *buffer, size_t capacity, int message_id, uint32_t version_mask)
{
    json_writer_t writer;
    if (!json_writer_init(&writer, buffer, capacity)) return -1;

    json_writer_append_format(&writer,
                              "{\"id\":%d,\"method\":\"mining.configure\",\"params\":[[\"version-rolling\"],{\"version-rolling.mask\":\"%08" PRIx32 "\",\"version-rolling.min-bit-count\":0}]}\n",
                              message_id, version_mask);
    return json_writer_finish(&writer);
}
...
static bool parse_mining_notify(cJSON *json, miner_job_t *job)
{
    ...
    if (params_count != 9 && params_count != 10) {
    ...
    // Accept the standard flag followed by an extension, and the legacy
    // ten-field layout whose extension precedes the final boolean flag.
    if (params_count == 10 && !cJSON_IsBool(clean_jobs_item)) {
        clean_jobs_item = cJSON_GetArrayItem(params, 9);
    }
    ...
    if (prefix_buffer == NULL || suffix_buffer == NULL ||
        c1_len > job->coinbase_prefix_capacity || c2_len > job->coinbase_suffix_capacity) {
...
static bool parse_set_version_mask(cJSON *json, StratumApiV1Message *message)
{
    ...
    if (!mask || !cJSON_IsString(mask) ||
        !is_hex_string(mask->valuestring, 8, 8, true)) {
...
static bool parse_set_extranonce(cJSON *json, StratumApiV1Message *message)
{
    ...
    if (extranonce_2_len < 0 || extranonce_2_len > MAX_EXTRANONCE_2_LEN) {
        ESP_LOGW(TAG, "Invalid extranonce_2_len %d", extranonce_2_len);
        return false;
    }
...
static bool parse_configure_result(cJSON *json, StratumApiV1Message *message)
{
    ...
    if (cJSON_IsFalse(version_rolling) || cJSON_IsString(version_rolling)) {
        message->response_success = false;
        return true;
    }
    if (!version_rolling || !cJSON_IsTrue(version_rolling) || !mask ||
        !cJSON_IsString(mask) || !is_hex_string(mask->valuestring, 8, 8, true)) {
...
static bool parse_message_id(const cJSON *json, StratumApiV1Message *message)
{
    ...
    if (!cJSON_IsNumber(id) || !isfinite(id->valuedouble) ||
        id->valuedouble < 0 || id->valuedouble > INT_MAX ||
