...
static bool save(void *context, power_target_t target)
{
...
static power_policy_t make_policy(board_fixture_t *b)
{
    power_policy_t policy;
    TEST_ASSERT_TRUE(power_policy_init(&policy, (power_operations_t){
...
static const power_target_t TARGET = {.voltage_mv = 2800, .frequency_mhz = 1200};
static const power_sample_t COOL = {.health = POWER_HEALTH_OK, .vreg_valid = true, .vreg_c = 70};
static void tick(power_policy_t *p, uint64_t now) { power_policy_step(p, now, TARGET, COOL, false, false, false); }

TEST_CASE("Power boot waits for the complete mining stack", "[power-management]")
{
    board_fixture_t b = {0}; power_policy_t p = make_policy(&b);
    tick(&p, 0); tick(&p, 100);
...
{
    board_fixture_t b = {0}; power_policy_t p = make_policy(&b); p.ready = true;
    tick(&p, 0);
    power_policy_step(&p, 100, TARGET, COOL, true, false, false);
    TEST_ASSERT_FALSE(p.running); TEST_ASSERT_FALSE(p.paused);
    TEST_ASSERT_TRUE(power_policy_pause(&p));
    tick(&p, 200); TEST_ASSERT_FALSE(p.running);
    TEST_ASSERT_EQUAL_STRING("SAXX", b.calls);
    TEST_ASSERT_TRUE(power_policy_resume(&p)); TEST_ASSERT_TRUE(p.running);
}
TEST_CASE("Pool recovery repeats startup without setting user pause", "[power-management]")
{
    ...
    tick(&p, 0);
    power_policy_step(&p, 100, TARGET, COOL, true, false, false);
    power_policy_step(&p, 200, TARGET, COOL, true, false, false);
    tick(&p, 300);
...
TEST_CASE("Failed startup and failed tuning cannot report running", "[power-management]")
{
    for (int fail_start = 0; fail_start <= 1; ++fail_start) {
        ...
        power_policy_t p = make_policy(&b); p.ready = true; tick(&p, 0);
        ...
        TEST_ASSERT_FALSE(power_policy_resume(&p));
    }
}
TEST_CASE("Failed shutdown stays unverified and cannot resume", "[power-management]")
{
    board_fixture_t b = {0}; power_policy_t p = make_policy(&b); p.ready = true; tick(&p, 0);
    b.fail_stop = true;
    TEST_ASSERT_FALSE(power_policy_pause(&p));
...
{
    board_fixture_t b = {.cancel_start = true}; power_policy_t p = make_policy(&b); p.ready = true;
    tick(&p, 0);
    ...
    TEST_ASSERT_TRUE(power_policy_resume(&p));
}
TEST_CASE("Overheat needs valid cooldown evidence and reduces settings once", "[power-management]")
{
    board_fixture_t b = {0}; power_policy_t p = make_policy(&b); p.ready = true;
    tick(&p, 0);
    power_sample_t hot = COOL; hot.health = POWER_HEALTH_OVERHEAT;
    power_policy_step(&p, 100, TARGET, hot, false, false, false);
    TEST_ASSERT_TRUE(p.cooling); TEST_ASSERT_FALSE(p.running);
    power_sample_t missing = COOL; missing.vreg_valid = false;
    power_policy_step(&p, 40000, TARGET, missing, false, false, true);
...
{
    board_fixture_t b = {0}; power_policy_t p = make_policy(&b); p.ready = true;
    power_policy_step(&p, 0, TARGET, COOL, false, false, true);
    TEST_ASSERT_TRUE(power_policy_pause(&p));
    tick(&p, 30000);
    TEST_ASSERT_TRUE(p.cooling); TEST_ASSERT_FALSE(p.running);
    TEST_ASSERT_TRUE(power_policy_maintenance(&p, POWER_OWNER_OTA, true));
    unsigned count = b.count; tick(&p, 60000); TEST_ASSERT_EQUAL_UINT(count, b.count);
    TEST_ASSERT_TRUE(power_policy_maintenance(&p, POWER_OWNER_OTA, false));
    p.pool_unavailable = true;
    TEST_ASSERT_TRUE(power_policy_resume(&p));
    power_policy_step(&p, 61000, TARGET, COOL, true, false, true);
...
{
    board_fixture_t b = {0}; power_policy_t p = make_policy(&b); p.ready = true; tick(&p, 0);
    TEST_ASSERT_TRUE(power_policy_maintenance(&p, POWER_OWNER_BRIDGE, true));
    TEST_ASSERT_FALSE(power_policy_maintenance(&p, POWER_OWNER_OTA, true));
    TEST_ASSERT_FALSE(power_policy_maintenance(&p, POWER_OWNER_OTA, false));
    TEST_ASSERT_FALSE(power_policy_resume(&p));
    unsigned count = b.count; tick(&p, 100); TEST_ASSERT_EQUAL_UINT(count, b.count);
    TEST_ASSERT_TRUE(power_policy_maintenance(&p, POWER_OWNER_BRIDGE, false));
    tick(&p, 200); TEST_ASSERT_FALSE(p.running);
    TEST_ASSERT_TRUE(power_policy_resume(&p));
}
TEST_CASE("Failed maintenance never grants update ownership", "[power-management]")
{
    board_fixture_t b = {.fail_maintenance = true}; power_policy_t p = make_policy(&b); p.ready = true;
    TEST_ASSERT_FALSE(power_policy_maintenance(&p, POWER_OWNER_OTA, true));
    TEST_ASSERT_EQUAL(POWER_OWNER_NONE, p.owner); TEST_ASSERT_TRUE(p.fault);
...
TEST_CASE("Powered-down ASIC sensors reset cooling time when still hot", "[power-management]")
{
    board_fixture_t b = {0}; power_policy_t p = make_policy(&b); p.ready = true;
    power_policy_step(&p, 0, TARGET, COOL, false, false, true);
    power_sample_t hot = COOL; hot.off_asic_sensor_required = true; hot.asic_valid = true; hot.asic_c = 50;
    power_policy_step(&p, 30000, TARGET, hot, false, false, true);
...
TEST_CASE("Overheat reduction clamps to board limits and save failures stay off", "[power-management]")
{
    for (int fail = 0; fail <= 1; ++fail) {
        board_fixture_t b = {.fail_save = fail}; power_policy_t p = make_policy(&b); p.ready = true;
        power_target_t minimum = {.voltage_mv = 2100, .frequency_mhz = 800};
        power_policy_step(&p, 0, minimum, COOL, false, false, true);
        power_policy_step(&p, 30000, minimum, COOL, false, false, true);
...
{
    power_policy_t p;
    TEST_ASSERT_FALSE(power_policy_init(NULL, (power_operations_t){0}));
    TEST_ASSERT_FALSE(power_policy_init(&p, (power_operations_t){0}));
}

TEST_CASE("Overheat discovered during startup enters the shared cooling path", "[power-management]")
{
    board_fixture_t b = {.hot_start = true}; power_policy_t p = make_policy(&b); p.ready = true;
    tick(&p, 0);
...
TEST_CASE("Failed maintenance release returns to a fault without orphaned ownership", "[power-management]")
{
    ...
    TEST_ASSERT_FALSE(power_policy_maintenance(&p, POWER_OWNER_OTA, false));
    TEST_ASSERT_EQUAL(POWER_OWNER_NONE, p.owner);
...
{
    board_fixture_t b = {0}; power_policy_t p = make_policy(&b); p.ready = true;
    power_policy_step(&p, 100, TARGET, COOL, false, false, true);
    power_sample_t sample = COOL;
    sample.off_asic_sensor_required = true;
    power_policy_step(&p, 30100, TARGET, sample, false, false, true);
...
TEST_CASE("Shared cooling preserves a BM1373 target below 400 MHz", "[power-management]")
{
    ...
    power_target_t target = {.voltage_mv = 1000, .frequency_mhz = 327};
    power_policy_step(&p, 0, target, COOL, false, false, true);
    power_policy_step(&p, 30000, target, COOL, false, false, true);
...
TEST_CASE("Fatal board health retries an unverified shutdown without automatic restart", "[power-management]")
{
    board_fixture_t b = {0}; power_policy_t p = make_policy(&b); p.ready = true;
    tick(&p, 0);
    power_sample_t bad = COOL; bad.health = POWER_HEALTH_FAULT;
    b.fail_stop = true;
    power_policy_step(&p, 100, TARGET, bad, false, false, false);
    TEST_ASSERT_TRUE(p.fault); TEST_ASSERT_FALSE(p.stopped);
    b.fail_stop = false;
    tick(&p, 200);
...
{
    board_fixture_t b = {0}; power_policy_t p = make_policy(&b);
    TEST_ASSERT_FALSE(power_policy_maintenance(&p, POWER_OWNER_NONE, true));
    TEST_ASSERT_FALSE(power_policy_maintenance(&p, (power_owner_t)99, true));
