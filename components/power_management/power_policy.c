...
static bool stop(power_policy_t *policy)
{
...
static bool cancelled(power_policy_t *policy)
{
    return policy->operations.cancelled != NULL &&
        policy->operations.cancelled(policy->operations.context);
}

static void begin_cooling(power_policy_t *policy, uint64_t now_ms, power_target_t target)
{
    policy->cooling = true;
...
bool power_policy_init(power_policy_t *policy, power_operations_t operations)
{
    ...
    *policy = (power_policy_t){.operations = operations};
    return true;
}
...
{
    policy->paused = true;
    if (policy->owner != POWER_OWNER_NONE) return false;
    return stop(policy);
}

bool power_policy_resume(power_policy_t *policy)
{
    if (!policy->ready || policy->fault || policy->owner != POWER_OWNER_NONE) return false;
    policy->paused = false;
    if (policy->cooling || policy->pool_unavailable) return true;
    if (policy->running) return true;
    power_start_result_t started = policy->operations.start(policy->operations.context);
    if (started == POWER_START_OVERHEAT && !cancelled(policy)) {
    ...
    if (started != POWER_START_OK || cancelled(policy)) {
...
{
    if (owner <= POWER_OWNER_NONE || owner > POWER_OWNER_RESTART) return false;
    if (acquire) {
        if (policy->owner != POWER_OWNER_NONE) return false;
        policy->paused = true;
        policy->running = false;
                       ...
                       bool persisted_overheat)
{
    ...
    if (policy->owner != POWER_OWNER_NONE) return;

    if (hardware_fault || sample.health == POWER_HEALTH_FAULT) {
        policy->fault = true;
    }
    if (policy->fault) {
    ...
    if (!policy->cooling && (sample.health == POWER_HEALTH_OVERHEAT || persisted_overheat)) {
    ...
    if (policy->cooling) {
        ...
        if (!policy->reduced_target_saved) {
            power_target_t reduced = policy->cooling_target;
    ...
    if (!policy->running && !power_policy_resume(policy)) return;
