...
TEST_CASE("SV2 parse new extended mining job direct decoding", "[sv2]")
{
    ...
    job.coinbase_prefix_capacity = sizeof(p_buf);
    job.coinbase_suffix = s_buf;
    job.coinbase_suffix_capacity = sizeof(s_buf);
    ...
    TEST_ASSERT_EQUAL(-1, sv2_parse_new_extended_mining_job(
        payload, pos, &channel_id, &job, &has_min_ntime, &version_rolling_allowed));
    TEST_ASSERT_EACH_EQUAL_UINT8(0xa5, s_buf, sizeof(s_buf));

    job.coinbase_prefix_capacity = 3;
    memset(p_buf, 0xa5, sizeof(p_buf));
    TEST_ASSERT_EQUAL(-1, sv2_parse_new_extended_mining_job(
        payload, pos, &channel_id, &job, &has_min_ntime, &version_rolling_allowed));
    TEST_ASSERT_EACH_EQUAL_UINT8(0xa5, p_buf, sizeof(p_buf));
}
