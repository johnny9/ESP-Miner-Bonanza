...
bool BM1373_send_work(GlobalState *GLOBAL_STATE, const asic_job_t *template)
{
    ...
    if (!asic_job_store_store_slot(&GLOBAL_STATE->asic_job_store, job.job_id,
                                   template, NULL)) {
        ESP_LOGE(TAG, "Unable to store work slot 0x%02X", job.job_id);
        return false;
    }
    ...
    _send_BM1373((TYPE_JOB | GROUP_SINGLE | CMD_WRITE), (uint8_t *)&job, sizeof(bm13xx_job_packet_t), BM1373_DEBUG_WORK);
    return true;
}
