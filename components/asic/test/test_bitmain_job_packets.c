...
static void assert_saved_job(bool (*snapshot)(uint8_t, asic_job_t *),
                             uint8_t slot, const asic_job_t *job)
{
    asic_job_t saved;
          ...
          "[asic][job-packet][characterization]")
{
    ...
    for (size_t index = 0; index < BM13XX_HARNESS_DRIVER_COUNT; ++index) {
        ...
        asic_job_t job_work = *job;
        TEST_ASSERT_TRUE(driver->send_work(state, &job_work));
        ...
        assert_saved_job(bm13xx_harness_snapshot, expected[index].job_id, job);
        free(job);
        bm13xx_harness_end();
    }
}
          ...
          "[asic][job-packet][version-rolling][characterization]")
{
    ...
    asic_job_t *job = make_job();
    asic_job_t job_work = *job;
    TEST_ASSERT_TRUE(bm1397_harness_driver.send_work(state, &job_work));
    ...
    assert_saved_job(bm1397_harness_snapshot, 4, job);
    free(job);
...
TEST_CASE("Bitmain occupied work slots retain owned common replacements",
          "[asic][job-store][characterization]")
{
    ...
    for (size_t index = 0; index < BM13XX_HARNESS_DRIVER_COUNT; ++index) {
        ...
        asic_job_t first_work = *first;
        TEST_ASSERT_TRUE(driver->send_work(state, &first_work));
        ...
        asic_job_t replacement_work = *replacement;
        TEST_ASSERT_TRUE(driver->send_work(state, &replacement_work));

        TEST_ASSERT_EQUAL_HEX8_MESSAGE(
            replacement_id, bm13xx_harness_packet(1)->bytes[4], driver->name);
        assert_saved_job(bm13xx_harness_snapshot, replacement_id, replacement);
        free(first);
        free(replacement);
        bm13xx_harness_end();
    }
    ...
    asic_job_t *first = make_job();
    asic_job_t first_work = *first;
    TEST_ASSERT_TRUE(bm1397_harness_driver.send_work(state, &first_work));
    ...
    asic_job_t *replacement = make_job();
    asic_job_t replacement_work = *replacement;
    TEST_ASSERT_TRUE(bm1397_harness_driver.send_work(state, &replacement_work));

    TEST_ASSERT_EQUAL_HEX8(replacement_id, bm1397_harness_packet(1)->bytes[4]);
    assert_saved_job(bm1397_harness_snapshot, replacement_id, replacement);
    free(first);
    free(replacement);
    bm1397_harness_end();
}
          ...
          "[asic][job-packet][version-rolling]")
{
    ...
    TEST_ASSERT_TRUE(bm1397_harness_driver.send_work(state, job));
    free(job);
