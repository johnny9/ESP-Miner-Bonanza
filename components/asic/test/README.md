...
| --- | --- |
| `test_bitmain_job_packets.c` | Runs the BM13xx and BM1397 drivers with the common owned-job store. It checks work packets, slot replacement, share and register responses, inactive jobs, and repeated nonces. |
| `test_version_rolling.c` | Runs the BM13xx drivers, common owned-job store, and SV1 and SV2 share encoders. It checks version-mask commands, driver setup, response decoding, write retries, and submitted version fields. |
| `test_asic_result_task.c` | Runs the ASIC result task with the common owned-job store and mining checks. It checks owned job snapshots, all job protocols, register routing, unavailable slots, share thresholds, self-test results, and repeated results. |

## Unit tests

| File | Unit behavior |
| --- | --- |
| `test_bitmain_job_building.c` | Converts common work through the real Bitmain builder; keeps header and midstate assertions within the ASIC component. |
| `test_common_work_contract.c` | Owned snapshot lifetime, generated-handle reuse, and clean invalidation. |
...
## Bonanza common interface

See [the integration contract](../../../doc/asic-common-interface.md) for the
mapping to the planned common ASIC interface and intentional baseline differences.
The same cases run under native sanitizers and QEMU. QEMU uses the full firmware
state; native test instances use the explicit shared view in `job_test_state.h`.
The result fixture also verifies actual header time and retired pool generations.
