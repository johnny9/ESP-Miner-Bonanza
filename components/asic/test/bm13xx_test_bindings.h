...
esp_err_t bm13xx_fake_receive_work(uint8_t *buffer, int size, uint64_t *timestamp_us);
int bm13xx_stub_count_chips(uint16_t count, uint16_t chip_id, int response_length);
int bm13xx_stub_count_chips_with_alias(uint16_t count, uint16_t chip_id,
                                      uint16_t alias, int response_length);
