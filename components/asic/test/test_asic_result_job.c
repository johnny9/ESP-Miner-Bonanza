...
static void replace_slot(GlobalState *state, asic_job_t *replacement)
{
    ...
    TEST_ASSERT_EQUAL_INT(0, pthread_mutex_trylock(&state->asic_job_store.lock));
    TEST_ASSERT_EQUAL_INT(0, pthread_mutex_unlock(&state->asic_job_store.lock));
    if (replacement != NULL) {
        TEST_ASSERT_TRUE(asic_job_store_store_slot(&state->asic_job_store, 0x10, replacement, NULL));
    } else {
        TEST_ASSERT_TRUE(asic_job_store_release(&state->asic_job_store, 0x10));
    }
}
                              ...
                              uint32_t nonce, uint32_t rolled_version)
{
    ...
    TEST_ASSERT_EQUAL_DOUBLE(mining_test_nonce_value(expected, nonce, expected->ntime, rolled_version),
                             mining_test_nonce_value(&result->job, result->nonce, result->job.ntime, result->rolled_version));
}
